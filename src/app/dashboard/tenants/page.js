...
import {
  ...
  useCalculatePayableAmount,
} from "@/hooks/tenants/useCreateTenantProfile";
...
import { useAdminAddress } from "@/hooks/tenants/useAdminAddress";
import { ExplorerLink, getToken } from "@/config/networks";
import { TEST_CHAINID } from "@/config/constants";
...
export default function TenantRequestPage() {
  const { address, isConnected } = useAccount(); // Check if user is connected
  const { sendTransaction } = useSendTransaction();
  const [detailsSubmitted, setDetailsSubmitted] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [isConfirmPurchaseLoading, setIsConfirmPurchaseLoading] =
    useState(false);
  ...
  const { mutate: createTenantProfile, isPending: isSubmittingProfile } =
    useCreateTenantProfile();
  ...
  const { mutate: getAdminAddress } = useAdminAddress();
  const { data: sendHash, writeContract: writeSend } = useWriteContract();
  const { data: sendTxConfirms, refetch: refetchSendTxConfirm } =
    useTransactionConfirmations({
      hash: sendHash,
    });
  ...
  }, [refetchSendTxConfirm, sendTxConfirms]);

  const handleSend = () => {
    writeSend({
      address: getToken(TEST_CHAINID, "usdc").address,
      abi: abis.erc20,
      functionName: "transfer",
      args: [adminAddress, parseUnits(formData.payableAmount, 6)],
    });
  };
    ...
    getAdminAddress(
      {},
      {
        onSuccess: (response) => {
          ...
          setAdminAddress(response.address);
        },
        onError: (error) => {
          console.error("Gettin admin address failed:", error);
        },
      }
    );
  }, [getAdminAddress]);
  ...
  const handleSubmitDetails = (e) => {
    ...
    const payload = {
      ...
      usdPrice: 1.0, // You may need to get this dynamically
    ...
    >
      ...
      <form onSubmit={handleSubmitDetails} className="space-y-6">
        <div>
              ...
              <div key={key} className="flex items-center gap-4">
                ...
                <label htmlFor={key} className="text-sm">
                  {key === "acceptedTerms" ? (
                    ...
                    <>
                      I consent to Rentsback storing and using my personal data
          ...
          {detailsSubmitted && (
            <div className="flex flex-col my-8 border-t-1">
              <p className="text-xl font-bold my-4">Payment gateway</p>

              <div className="flex flex-col rounded-md border border-gray p-5 gap-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {sendHash && (
                    <>
                      <Link
                        href={`${ExplorerLink(TEST_CHAINID)}tx\\${sendHash}`}
                        className="inline-flex items-center bg-white text-blue-600 py-4 font-semibold hover:underline transition-colors"
                      >
                        {`TxHash: ${sendHash}`}
                      </Link>
                      <div className="flex text-xl items-center justify-center">
                        {sendTxConfirms > 3
                          ? "Payment Verified"
                          : `${sendTxConfirms} confirmations done`}
                      </div>
                    </>
                  )}
                </div>
                <div className="flex justify-center">
                  {!sendHash && (
                    <MotionButton
                      ...
                      type="button"
                      onClick={handleSend}
                      className="btn-primary w-1/3 bg-[#130fa3] text-white py-2 px-4 rounded hover:bg-[#0a0879] transition-colors flex items-center justify-center"
                    >
                      Send
                    </MotionButton>
                  )}
          ...
          <div className="flex gap-2 space-y-4 mt-5">
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <MotionButton
                  whileHover={{
                    scale: detailsSubmitted && paymentPassed ? 1.02 : 1,
                  }}
                  whileTap={{
                    scale: detailsSubmitted && paymentPassed ? 0.98 : 1,
                  }}
                  type="button"
                  className={`btn-primary w-full py-2 px-4 rounded transition-colors flex items-center justify-center ${
                    detailsSubmitted && paymentPassed
                      ? "bg-[#130fa3] text-white hover:bg-[#0a0879]"
                      : "bg-gray-300 text-gray-500 cursor-not-allowed"
                  }`}
                  disabled={
                    !detailsSubmitted || !paymentPassed || isSubmittingProfile
                  }
                >
                  {isSubmittingProfile ? (
